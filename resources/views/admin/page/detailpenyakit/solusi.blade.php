...
@section('title' , 'Detail Penyakit | Solusi' , 'active')
			...
			<div class="content">
				<div class="page-inner">
					<div class="page-header">
						<h4 class="page-title">Data Detail Penyakit | Solusi</h4>
						<ul class="breadcrumbs">
							...
							<li class="nav-item">
								<a href="/admin/detailpenyakit_solusi">Solusi</a>
							</li>
						</ul>
					</div>
					<div class="row">
							...
							<div class="card">
								<div class="card-header">
									<div class="d-flex align-items-center">
										<h4 class="card-title"><b>Detail Penyakit | Solusi</b></h4>
								...
								<div class="card-body">
													...
													<div class="row">
														<div class="col-sm-12">
															<div class="form-group form-group-default">
																<form action="{{ route('detailpenyakit_solusi.store') }}" method="POST">
																		@csrf
																	<div>
																	...
																	<br>
																	<label>Solusi</label>
																	<select id="id_gejala" class="form-control" name="id_solusi" style="border: 1px solid #f1f1f1; line-height:40px; padding:10px;">
																		<option value="">Solusi</option>
																		@foreach ($solusi as $data)
																			{{-- @foreach ($data->dpGejala as $dt) --}}
																			<option value="{{ $data->id }}">{{ $data->nama_solusi }}</option>
																			{{-- @endforeach --}}
																		@endforeach
									...
									<div class="table-responsive">
										<table id="add-row" class="display table table-striped table-hover" >
											...
											<tbody>
												@foreach ($datadetailsolusi as $dt)
													
												<tr>
													<td>{{ $loop->iteration }}</td>
													<td>{{ $dt->penyakit['nama_penyakit'] }}</td>
													<td>{{ $dt->solusi['nama_solusi'] }}</td>
													<td>
														<div class="form-button-action">
																		...
																		<div class="modal-content">
																			...
																			<div class="modal-body">
																					<div class="row">
																						<div class="col-sm-12">
																							<div class="form-group form-group-default">
																								<form action="{{ route('detailpenyakit_solusi.update', $dt->id) }}" method="POST">
																									@method('put')
																									@csrf
																									<div>
																										<label>Nama Penyakit</label>
																										...
																										<select id="id_gejala" class="form-control" name="id_gejala" style="border: 1px solid #f1f1f1; line-height:40px; padding:10px;">
																											<option value="" hidden>{{ $dt->solusi['nama_solusi'] }}</option>
																											@foreach ($solusi as $data)
																												{{-- @foreach ($data->dpGejala as $dt) --}}
																												<option value="{{ $data->id }}">{{ $data->nama_solusi }}</option>
																												{{-- @endforeach --}}
															...
															<form action="{{ route('detailpenyakit_solusi.destroy', $dt->id) }}" method="POST">
																@csrf
																@method('delete')
