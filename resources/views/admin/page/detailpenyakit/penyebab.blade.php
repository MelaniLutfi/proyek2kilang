...
@section('title' , 'Detail Penyakit | Penyebab' , 'active')
			...
			<div class="content">
				<div class="page-inner">
					<div class="page-header">
						<h4 class="page-title">Data Detail Penyakit | Penyebab</h4>
						<ul class="breadcrumbs">
							...
							<li class="nav-item">
								<a href="/admin/detailpenyakit_penyebab">Penyebab</a>
							</li>
						</ul>
					</div>
					<div class="row">
							...
							<div class="card">
								<div class="card-header">
									<div class="d-flex align-items-center">
										<h4 class="card-title"><b>Detail Penyakit | Penyebab</b></h4>
								...
								<div class="card-body">
													...
													<div class="row">
														<div class="col-sm-12">
															<div class="form-group form-group-default">
																<form action="{{ route('detailpenyakit_penyebab.store') }}" method="POST">
																		@csrf
																	<div>
																	...
																	<select id="id_gejala" class="form-control" name="id_penyebab" style="border: 1px solid #f1f1f1; line-height:40px; padding:10px;">
																		<option value="">Pilih Penyebab</option>
																		@foreach ($penyebab as $data)
																			{{-- @foreach ($data->dpGejala as $dt) --}}
																			<option value="{{ $data->id }}">{{ $data->nama_penyebab }}</option>
																			{{-- @endforeach --}}
																		@endforeach
									...
									<div class="table-responsive">
										<table id="add-row" class="display table table-striped table-hover" >
											...
											<tbody>
												@foreach ($datadetailpenyebab as $dt)
													
												<tr>
													<td>{{ $loop->iteration }}</td>
													<td>{{ $dt->penyakit['kode_penyakit'] }}-{{ $dt->penyakit['nama_penyakit'] }}</td>
													<td>{{ $dt->penyebab['kode_penyebab'] }}-{{ $dt->penyebab['nama_penyebab'] }}</td>
													<td>
														<div class="form-button-action">
																		...
																		<div class="modal-content">
																			...
																			<div class="modal-body">
																					<div class="row">
																						<div class="col-sm-12">
																							<div class="form-group form-group-default">
																								<form action="{{ route('detailpenyakit_penyebab.update', $dt->id) }}" method="POST">
																									@method('put')
																									@csrf
																									<div>
																										<label>Nama Penyakit</label>
																										<select id="id_penyakit" class="form-control" name="id_penyakit" style="border: 1px solid #f1f1f1; line-height:40px; padding:10px;">
																											<option value="{{ $dt->id_penyakit }}">{{ $dt->penyakit['nama_penyakit'] }}</option>
																										...
																										<label>Gejala</label>
																										<select id="id_gejala" class="form-control" name="id_penyebab" style="border: 1px solid #f1f1f1; line-height:40px; padding:10px;">
																											<option value="" hidden>{{ $dt->penyebab['nama_penyebab'] }}</option>
																											@foreach ($penyebab as $data)
																												{{-- @foreach ($data->dpGejala as $dt) --}}
																												<option value="{{ $data->id }}">{{ $data->nama_penyebab }}</option>
																												{{-- @endforeach --}}
															...
															<form action="{{ route('detailpenyakit_penyebab.destroy', $dt->id) }}" method="POST">
																@csrf
																@method('delete')
